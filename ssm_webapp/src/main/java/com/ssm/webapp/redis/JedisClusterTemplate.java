...
public class JedisClusterTemplate {
    ...
    private static Logger log = LoggerFactory.getLogger(JedisClusterTemplate.class);
    ...
    /**
     * 添加,如果执行该命令之前，该Key并不存在，该命令将会创建一个新的Set，
     * 此后再将参数中的成员陆续插入.如果该Key的Value不是Set类型，该命令将返回相关的错误信息.
     * 
     * @param key
     * @param member
     * @return
     ...
     * <br>
     * 如果start大于该Sorted-Set中的最大索引值，或start >
     * stop，此时一个空集合将被返回.如果stop大于最大索引值，该命令将返回从start到集合的最后一个成员.
     * 
     * @param key
     * @param start
     * @param end
     ...
     * max的成员，其中返回的成员是按照其分数从低到高的顺序返回，如果成员具有相同的分数，则按成员的字典顺序返回.
